...
const AdminPage = () => {
  ...
  useEffect(() => {
    const { data } = session;
    console.log(session, 'sessionsessionsessionsessionsession')
    if (session?.data?.user) {
      console.log('🚀 ~ useEffect ~ session:', session)
      console.log('🚀 ~ useEffect ~ data:', data)
      axiosInstance
        .get("/project/listByInversor", {
  ...
  return (
    <LayoutBackOffice>
      ...
      <Container style={{ marginTop: "82px" }} className="py-5">
        <h1 className="display-4">Mis inversiones</h1>
        <p className="text-muted">Explora y administra tus inversiones de crowdfunding</p>
            ...
            {filteredProjects.length > 0 ? (
              filteredProjects.map((project, index) => (
                <tr key={project.id}>
                  ...
                  <td>{formatPrice(project.transaction.totalSum / 100) }</td>
                  <td>
                    {formatPrice(project.transaction.totalSum / 100) }
                  </td>
                </tr>
              ))
            ) : (
